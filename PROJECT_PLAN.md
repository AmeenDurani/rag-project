...
- [x] GitHub Actions: lint (ruff) + pytest on push/PR
- [x] ~~README: architecture diagram, setup steps, eval results table (before/after), a design-decisions section~~ — scope changed mid-Day-4: split into a short README (setup only) + a hosted technical whitepaper (architecture, design decisions, the full eval-driven chunking story with real charts, limitations). See `progress/add-technical-whitepaper.md`.
- [ ] Stretch: Streamlit/Gradio UI — question box, shows retrieved chunks + answer + sources

**Milestone:** containerized, CI-checked, documented project ready to link from a resume. **Status: core complete, stretch UI open.** Dockerfile - see `progress/add-dockerfile.md` (Gunicorn + Uvicorn worker process manager for multi-core scaling, `data/` mounted as a volume so uploaded PDFs persist across restarts; verified the equivalent CMD and a clean non-editable `pip install .` locally, since Docker itself isn't installed on this machine). GitHub Actions CI - see `progress/add-github-actions-ci.md` (lint + pytest jobs, no secrets needed since the test suite is fully hermetic; verified the exact commands pass from a clean install before pushing the workflow). Documentation - see `progress/add-technical-whitepaper.md`: README rewritten as a short setup-only landing page; a separate hosted whitepaper (`whitepaper/`, plain static HTML/CSS/JS) carries the architecture, design decisions, and the full chunking-iteration narrative with real charts built from `eval/results/*.json` - deployed live at `https://rag-project-tawny.vercel.app/`, re-verified against the live URL (not just the local copy) with a headless browser. Stretch UI is the only Day 4 item still open.
