...
**Day 3 complete.** FastAPI service layer (`add-fastapi-service-layer.md`) - `/ingest` (file upload, persists to `data/`), `/query`, `/health` under `src/api/`. Config (pydantic-settings, fail-fast on missing keys) was already done back on Day 1. Unit test suite (`add-unit-test-suite.md`) - 23 hermetic tests, no real API calls or model downloads, ~2s runtime. Logging/tracing (`add-logging-and-tracing.md`) - structured JSON logs, contextvars-based request IDs, per-stage latency, per-request estimated cost - verified against live Pinecone/Anthropic, not just unit-tested.

**Day 4, two pieces done.** Dockerfile (`add-dockerfile.md`) - Gunicorn + Uvicorn-worker process manager (env-var configurable worker count, default 2) for multi-core scaling and crash isolation; `data/` mounted as a volume so uploaded PDFs persist across restarts. Docker itself isn't installed on this machine, so the actual build/run is unverified - the equivalent CMD and packaging step were verified directly instead (see the entry for specifics). GitHub Actions CI (`add-github-actions-ci.md`) - parallel lint + pytest jobs, no secrets needed (hermetic test suite), verified the exact commands pass from a clean install before pushing. README and the stretch UI are still open.
...
- [`add-logging-and-tracing.md`](add-logging-and-tracing.md) — structured JSON logs, contextvars-based request IDs, per-stage latency (embed_and_retrieve/generate, extract_and_chunk/embed/upsert), and per-request estimated dollar cost; closes out Day 3
- [`add-dockerfile.md`](add-dockerfile.md) — first Day 4 piece: Dockerfile using Gunicorn + Uvicorn workers for multi-core scaling and crash isolation, data/ mounted as a volume for upload persistence; Docker itself unavailable on this machine, so the CMD and packaging step were verified directly instead of via a real `docker build`
- [`add-github-actions-ci.md`](add-github-actions-ci.md) — parallel lint (ruff) + pytest jobs on push/PR to main, no secrets needed since the test suite is fully hermetic; verified the exact commands pass from a clean install before pushing the workflow
